...
**系统定名**：基于轻量级 ReAct 架构的高校机房智能运维与资产管理系统（LabOps-Agent）  
**面向专业**：计算机科学与技术 / 软件工程 本科毕业设计  
**开发耗时预算**：24 小时（严格控制在 20-30 小时内，确保考编备考时间绝对充足）
...
### 1.2 绝不被质疑为“API 套壳”的核心设计
在毕业设计答辩中，评委最反感的是“仅输入一段话丢给模型输出一段话”的无状态系统。本项目通过以下 4 项硬核软件工程指标筑起技术壁垒：
...
├── README.md                        # 项目启动与说明文档
└── docs/
    └── architecture_plan.md         # 架构设计与毕业论文对应章节底稿
```

---

## 四、 MVP 极速开发计划（严格 24 小时预算拆解）

针对考编日程，将开发任务切分为 5 个清晰阶段，每阶段平均耗时 4-5 小时。完成后立即“代码封板”，不再做任何扩展。
...
| **阶段 5：Docker 容器化、封板与材料归档** | 编写 Dockerfile、Docker-compose 与中期检查录屏 | 1. 编写 FastAPI 与 Vue 前端的 `Dockerfile`<br>2. 配置 `nginx.conf` 与根目录 `docker-compose.yml`，在本地/VPS 测试 `docker compose up -d` 一键启动<br>3. 录制 5 分钟标准操作全流程演示视频（备用演示防车祸）并截取核心架构图 | 4 小时 | **彻底代码封板**。只需一个命令即可在任何机器拉起，具备完备的可运行中期检查支撑材料。 |
| **合计** | **全流程闭环** | **避开所有第三方繁琐框架，纯粹工程落地** | **24 小时** | 顺利进入考编全脱产冲刺状态！ |

---

## 四、 答辩核心问答预设（答辩锦囊）

| 答辩老师可能的提问 | 推荐的标准应答思路（凸显软工素养） |
| :--- | :--- |
| **“你这个系统是不是就是调了一下 DeepSeek 的接口做个聊天框？”** | **从三方面否定**：<br>1. **业务操作闭环**：系统实现了 Tool Calling 对 SQLite 资产与工单数据库的真实读写，对话不是终点，产生业务流转才是终点；<br>2. **自主决策 RAG**：Agent 会主动结合机房规章知识库与实时监控指标判断故障严重度，而非通用闲聊；<br>3. **全自主研发状态机**：未使用现成黑盒智能体框架，自主实现了完整的 ReAct 状态循环与 SSE 分块解析协议。 |
| **“为什么不使用 LangChain 或者 AutoGen 多智能体框架？”** | **从工程可靠性与 KISS 原则应答**：<br>1. 多智能体系统在现实运维场景存在严重的不确定性与 Token 消耗黑盒，极易死循环；<br>2. 本项目聚焦垂直机房运维，采用成熟的“单路由 Agent + 确定性工具集”，代码执行链路清晰可控，响应延迟低，更契合企业级高可用运维诉求。 |
| **“你的 RAG 向量知识库是如何避免模型幻觉的？”** | **从 Prompt 与溯源机制应答**：<br>1. 系统通过 Top-K 向量相似度检索出相关条款，并在 Prompt 中施加严格的“基于检索事实回答，未知则说明”约束；<br>2. 前端展示溯源引用的规章文档名称及原文片段，确保每一句运维建议皆有规章可循。 |
