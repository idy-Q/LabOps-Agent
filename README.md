# 面向高校实验室的轻量化智能自治运维系统 (LabOps-Agent)

LabOps-Agent 是一个**面向高校实验室的轻量化智能自治运维系统**。系统采用自研 ReAct（Reasoning + Acting）推理循环与 RAG 规程知识检索，集成了设备运行监控、故障报修闭环、管理规范问答以及设备借用登记等功能。
