...
│   ├── tests/                     # 自动化回归测试套件 (覆盖工具集、调度引擎、RAG、用户鉴权、端到端接口，101 项全绿)
...
│       ├── 01_database_design/    # 数据库建表 DDL (schema.sql) 与 5 张核心表逻辑设计数据字典
│       ├── 02_screenshots/        # 系统核心功能界面规范截图说明
│       ├── 03_test_cases/         # 各阶段 101+ 项测试用例执行报告（含用户认证与RBAC，论文直接复用）
