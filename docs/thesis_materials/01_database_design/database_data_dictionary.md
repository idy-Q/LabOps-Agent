...
## 表 4-5 系统用户与角色权限表 (users)

> **答辩创新点**：支撑系统基于角色的机房资产借还防冒名控制（RBAC 规范）。前端自动读取当前登录会话之真实姓名与所属院系部门，在设备借调办理时强制设为只读并实时校验角色，杜绝无资质学生随意借出核心交换机或超算设备。

| 字段名称            | 字段类型         | 允许为空 | 默认值               | 约束 / 索引       | 业务含义说明                                                                 |
|:--------------- |:------------ |:----:|:-----------------:|:-------------:|:--------------------------------------------------------------------- |
| `id`            | INTEGER      | 否    | 自增                | 主键 (PK)       | 用户主键唯一标识符                                                             |
| `username`      | VARCHAR(64)  | 否    | -                 | 唯一索引 (UNIQUE) | 登录工号或学生学号 (如 `admin`, `teacher_li`, `student_chen`)                     |
| `password_hash` | VARCHAR(128) | 否    | -                 | -             | 加盐安全密码哈希值 (基于 SHA-256 与特定加密盐计算得到的 64 位 Hex 字符串)                    |
| `real_name`     | VARCHAR(64)  | 否    | -                 | -             | 用户真实姓名 (如 `王主管`, `李老师`, `陈同学`)                                         |
| `role`          | VARCHAR(32)  | 否    | 'TEACHER'         | -             | 用户身份角色枚举 (`ADMIN` 超级管理员 / `TEACHER` 实验指导教师 / `STUDENT` 实训学生)        |
| `department`    | VARCHAR(128) | 否    | -                 | -             | 所属教研室、行政部门或教学班级 (如 `网络中心运维部`, `物联网工程教研室`, `计科2201班`)         |
| `phone`         | VARCHAR(32)  | 是    | NULL              | -             | 联系电话或应急通讯号码 (如 `13800000001`)                                        |
| `created_at`    | DATETIME     | 否    | CURRENT_TIMESTAMP | -             | 账户注册生成时间戳                                                            |

### 权限与安全激活机制说明
1. **密码哈希防泄露**：全系统禁止明文存储任何用户口令，统一采用 `hashlib.sha256("labops_secure_salt_2026:" + password)` 安全加盐哈希，彻底杜绝数据库物理泄露导致的彩虹表撞库风险。
2. **管理员专属安全口令 (提权防御)**：系统设定管理员专属安全激活口令为 `admin666`。当在前端注册申请 `ADMIN` 角色时，必须提供此安全口令，否则服务端 100% 拒绝授予管理员权限。
3. **防冒名借调业务闭环**：教师（TEACHER）与管理员（ADMIN）发起设备借出登记时，借用人及部门由系统自动读取并锁定只读；学生（STUDENT）角色下资产借用按钮自动置灰禁用，杜绝越权代借。
